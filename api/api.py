...
URL = "http://localhost:8001"  # URL нового model_api

# Инициализация приложения FastAPI
app = FastAPI()


# Эндпоинт для предсказания по текстовым от	зывам
@app.post("/predict/text", summary="Предсказание по текстовым отзывам",
		  description="""Принимает ключ платформы, ключ курса и текстовые отзывы. Отправляет данные на
          внешний сервис для получения предсказания и возвращает результат.""")
async def review_from_list(
	...
	try:
		data = {"text": " ".join(review)}  # Соединяем отзывы в один текст для предсказания
		async with httpx.AsyncClient() as client:
			response = await client.post(f"{URL}/predict/text", json=data)
...
@app.post("/predict/file", summary="Предсказание по загруженному файлу",
		  description="""Принимает загруженный CSV файл, ключ платформы и ключ курса. Отправляет
          файл на внешний сервис для получения предсказания и возвращает результат.""")
async def review_from_file(
	...
	# Отправляем файл и ключи платформы/курса
	try:
		data = {"platform_key": platform_key, "course_key": course_key}
		async with httpx.AsyncClient() as client:
			response = await client.post(f"{URL}/predict/file", files=files)
		response.raise_for_status()  # Проверка успешности запроса

		# Возвращаем сгенерированный CSV файл
		return StreamingResponse(response.aiter_bytes(), media_type="text/csv")
	except httpx.HTTPStatusError as e:
		raise HTTPException(status_code=response.status_code, detail="Unexpected error") from e
	except httpx.RequestError as e:
		raise HTTPException(status_code=500, detail="Internal server error") from e
...
@app.post("/add_user", summary="Добавление нового пользователя",
		  description="""Добавляет нового пользователя по ключу платформы и возвращает ключ
          пользователя. Требуется секретный ключ для авторизации.""")
async def add_user(
